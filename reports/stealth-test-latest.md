# Stealth Test Report

<<<<<<< Updated upstream
**Date:** 2026-02-27 15:57 UTC

## darwin-arm64

**Result:** 22/22 passed in 4.2s

| Test | Status | Time |
|------|--------|------|
| `test_navigator_webdriver_false` | PASS | 3.55s |
| `test_no_headless_chrome_ua` | PASS | 0.01s |
| `test_window_chrome_exists` | PASS | 0.00s |
| `test_plugins_present` | PASS | 0.01s |
| `test_languages_present` | PASS | 0.01s |
| `test_cdp_not_detected` | PASS | 0.01s |
| `test_hardware_concurrency_override` | PASS | 0.00s |
| `test_platform_override` | PASS | 0.00s |
| `test_worker_platform_matches_main` | PASS | 0.03s |
| `test_canvas_noise_applied` | PASS | 0.01s |
| `test_audio_context_noise` | PASS | 0.02s |
| `test_client_rect_noise_applied` | PASS | 0.01s |
| `test_cdp_prepare_stack_trace_not_triggered` | PASS | 0.01s |
| `test_cdp_not_detected_in_worker` | PASS | 0.02s |
| `test_no_playwright_globals` | PASS | 0.00s |
| `test_chrome_runtime_exists` | PASS | 0.01s |
| `test_webdriver_false_in_iframe` | PASS | 0.02s |
| `test_worker_values_consistent` | PASS | 0.02s |
| `test_webgl_vendor_override` | PASS | 0.03s |
| `test_webgl_renderer_override` | PASS | 0.01s |
| `test_client_hints_platform_consistent` | PASS | 0.01s |
| `test_screen_resolution_not_headless_default` | PASS | 0.29s |

## linux-amd64

**Result:** 20/22 passed in 8.6s

| Test | Status | Time |
|------|--------|------|
| `test_navigator_webdriver_false` | PASS | 7.86s |
=======
**Date:** 2026-02-27 16:06 UTC

## darwin-arm64

**Result:** 22/22 passed in 2.6s

| Test | Status | Time |
|------|--------|------|
| `test_navigator_webdriver_false` | PASS | 2.13s |
| `test_no_headless_chrome_ua` | PASS | 0.00s |
| `test_window_chrome_exists` | PASS | 0.00s |
| `test_plugins_present` | PASS | 0.00s |
| `test_languages_present` | PASS | 0.00s |
| `test_cdp_not_detected` | PASS | 0.00s |
| `test_hardware_concurrency_override` | PASS | 0.00s |
| `test_platform_override` | PASS | 0.00s |
| `test_worker_platform_matches_main` | PASS | 0.02s |
| `test_canvas_noise_applied` | PASS | 0.01s |
| `test_audio_context_noise` | PASS | 0.02s |
| `test_client_rect_noise_applied` | PASS | 0.00s |
| `test_cdp_prepare_stack_trace_not_triggered` | PASS | 0.00s |
| `test_cdp_not_detected_in_worker` | PASS | 0.01s |
| `test_no_playwright_globals` | PASS | 0.00s |
| `test_chrome_runtime_exists` | PASS | 0.00s |
| `test_webdriver_false_in_iframe` | PASS | 0.01s |
| `test_worker_values_consistent` | PASS | 0.01s |
| `test_webgl_vendor_override` | PASS | 0.02s |
| `test_webgl_renderer_override` | PASS | 0.01s |
| `test_client_hints_platform_consistent` | PASS | 0.01s |
| `test_screen_resolution_not_headless_default` | PASS | 0.26s |

## linux-amd64

**Result:** 20/22 passed in 6.1s

| Test | Status | Time |
|------|--------|------|
| `test_navigator_webdriver_false` | PASS | 4.58s |
>>>>>>> Stashed changes
| `test_no_headless_chrome_ua` | PASS | 0.00s |
| `test_window_chrome_exists` | PASS | 0.00s |
| `test_plugins_present` | PASS | 0.01s |
| `test_languages_present` | PASS | 0.00s |
| `test_cdp_not_detected` | PASS | 0.00s |
| `test_hardware_concurrency_override` | PASS | 0.00s |
| `test_platform_override` | PASS | 0.00s |
| `test_worker_platform_matches_main` | PASS | 0.01s |
| `test_canvas_noise_applied` | PASS | 0.01s |
| `test_audio_context_noise` | PASS | 0.01s |
| `test_client_rect_noise_applied` | PASS | 0.00s |
| `test_cdp_prepare_stack_trace_not_triggered` | PASS | 0.00s |
| `test_cdp_not_detected_in_worker` | PASS | 0.01s |
| `test_no_playwright_globals` | PASS | 0.00s |
| `test_chrome_runtime_exists` | PASS | 0.00s |
| `test_webdriver_false_in_iframe` | PASS | 0.01s |
| `test_worker_values_consistent` | PASS | 0.01s |
| `test_webgl_vendor_override` | SKIP | 0.00s |
| `test_webgl_renderer_override` | SKIP | 0.00s |
| `test_client_hints_platform_consistent` | PASS | 0.00s |
<<<<<<< Updated upstream
| `test_screen_resolution_not_headless_default` | PASS | 0.47s |

## live-darwin-arm64

**Result:** 4/4 passed in 38.3s

| Test | Status | Time |
|------|--------|------|
| `test_device_and_browser_info` | PASS | 13.44s |
| `test_browserscan` | PASS | 11.45s |
| `test_fingerprintjs` | PASS | 7.17s |
| `test_recaptcha_v3` | PASS | 6.18s |

## live-linux-amd64

**Result:** 4/4 passed in 33.6s

| Test | Status | Time |
|------|--------|------|
| `test_device_and_browser_info` | PASS | 11.11s |
| `test_browserscan` | PASS | 9.63s |
| `test_fingerprintjs` | PASS | 6.95s |
| `test_recaptcha_v3` | PASS | 5.88s |

**Total: 50/52 passed across 4 platform(s) in 84.7s**
=======
| `test_screen_resolution_not_headless_default` | PASS | 1.28s |

## live-darwin-arm64

**Result:** 4/4 passed in 33.7s

| Test | Status | Time |
|------|--------|------|
| `test_device_and_browser_info` | PASS | 12.47s |
| `test_browserscan` | PASS | 9.26s |
| `test_fingerprintjs` | PASS | 6.51s |
| `test_recaptcha_v3` | PASS | 5.36s |

## live-linux-amd64

**Result:** 4/4 passed in 33.7s

| Test | Status | Time |
|------|--------|------|
| `test_device_and_browser_info` | PASS | 11.73s |
| `test_browserscan` | PASS | 9.71s |
| `test_fingerprintjs` | PASS | 6.74s |
| `test_recaptcha_v3` | PASS | 5.43s |

**Total: 50/52 passed across 4 platform(s) in 76.1s**
>>>>>>> Stashed changes
